...
import com.dmspallas.retro.spacegame.Effect.Asteroid;
...
public class MainGameScreen implements Screen {
    ...
    public MainGameScreen (SpaceGame game) {
        ...
        playerRect = new CollisionEffect(0, 0, SHIP_WIDTH, SHIP_HEIGHT);
    ...
    public void render (float delta) {
        ...
        for (Bullet bullet : bullets) {
            for (Asteroid asteroid : asteroids) {
                if (bullet.getCollisionEffect().collisionWith(asteroid.getCollisionEffect())) {//Collision occured
                    bulletsToRemove.add(bullet);
                    asteroidsToRemove.add(asteroid);
        ...
        for (Asteroid asteroid : asteroids) {
            if (asteroid.getCollisionEffect().collisionWith(playerRect)) {
                asteroidsToRemove.add(asteroid);
                health -= 0.1;
